...
import { Purchase, AppSettings, Supplier, Warehouse } from '../types';
import { formatCurrency } from '../utils/formatters';

interface PurchaseListPrintProps {
    purchases: Purchase[];
...
export const PurchaseListPrint = React.forwardRef<HTMLDivElement, PurchaseListPrintProps>((props, ref) => {
    const { purchases, settings, suppliers, warehouses, period } = props;
    ...
    const totalBalance = purchases.reduce((sum, p) => sum + (p.grandTotal - p.paidAmount), 0);
        ...
        <div ref={ref} className="bg-white p-8 mx-auto text-black font-sans text-xs print:max-w-none print:w-[210mm] print:min-h-[297mm] print:shadow-none" style={{ minHeight: '210mm', width: '210mm' }}>
            ...
            <div className="flex justify-between items-start mb-8 border-b-2 border-gray-800 pb-4">
                ...
                <div className="text-right">
                    <h2 className="text-2xl font-black uppercase tracking-tighter">Journal des Achats</h2>
                    ...
                    <p className="text-xs font-medium text-gray-500">Nombre d'achats: {purchases.length}</p>
                </div>
            </div>
            ...
            <table className="w-full border-collapse">
                ...
                <tbody className="divide-y divide-gray-200">
                    {purchases.map((purchase, index) => {
                        ...
                        return (
                            <tr key={purchase.id} className="break-inside-avoid print:break-inside-avoid">
                                <td className="py-2 px-2 text-gray-500">{index + 1}</td>
                                <td className="py-2 px-2">{new Date(purchase.date).toLocaleDateString('fr-FR')}</td>
                                <td className="py-2 px-2 font-mono font-bold">{purchase.referenceNumber}</td>
                                <td className="py-2 px-2">{getSupplierName(purchase.supplierId)}</td>
                                <td className="py-2 px-2 text-[10px] uppercase">{getWarehouseName(purchase.warehouseId)}</td>
                                <td className="py-2 px-2 text-center text-[10px] uppercase">
                                    <span className={`px-2 py-1 rounded-full ${
                                        purchase.paymentStatus === 'Payé' ? 'bg-green-100 text-green-800' :
                                        purchase.paymentStatus === 'Partiel' ? 'bg-orange-100 text-orange-800' :
