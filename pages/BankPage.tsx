...
const BankPage: React.FC = () => {
    ...
    const totalFilteredDeposit = useMemo(() => {
        return filteredTransactions
            .filter(t => t.type === 'deposit')
            .reduce((sum, t) => sum + t.amount, 0);
    }, [filteredTransactions]);

    const totalFilteredWithdrawal = useMemo(() => {
        return filteredTransactions
            ...
            <div className="bg-white dark:bg-gray-800 shadow-2xl rounded-3xl border border-gray-100 dark:border-gray-700 overflow-hidden">
                <div className="overflow-x-auto">
                    <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        ...
                        <tfoot className="border-t-2 border-gray-200 dark:border-gray-700 bg-blue-50 dark:bg-blue-900/20">
                            <tr>
                                <td colSpan={3} className="px-6 py-4 text-right text-xs font-black uppercase text-gray-500 tracking-widest">Totaux</td>
                                <td className="px-6 py-4 text-right font-black text-red-600 text-sm">{formatCurrency(totalFilteredWithdrawal)}</td>
                                <td className="px-6 py-4 text-right font-black text-green-600 text-sm">{formatCurrency(totalFilteredDeposit)}</td>
                                <td colSpan={2}></td>
                            </tr>
                        </tfoot>
                    </table>
