...
const SuppliersPage: React.FC = () => {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            suppliersData.forEach(s => {
                if (typeof s.openingBalance !== 'number' || s.openingBalance > 1000000000) {
                    problematicValues.push({
                        ...
                        type: typeof s.openingBalance
                    });
                }
            });

            purchasesData.forEach(p => {
                if (typeof p.grandTotal !== 'number' || p.grandTotal > 1000000000) {
                    problematicValues.push({
                        purchase: p.id,
                        field: 'grandTotal',
                        value: p.grandTotal,
                        type: typeof p.grandTotal
                    });
                }
                if (typeof p.paidAmount !== 'number' || p.paidAmount > 1000000000) {
                    problematicValues.push({
                        purchase: p.id,
                        field: 'paidAmount',
                        value: p.paidAmount,
                        type: typeof p.paidAmount
                    });
                }
            });

            if (problematicValues.length > 0) {
                console.warn('Valeurs problématiques détectées:', problematicValues);
            }
            ...
            paymentsData.forEach(p => {
                ...
                if (purchaseId && purchaseId.startsWith('OPENING_BALANCE_')) {
                    ...
                    if (balanceMap[supplierId] !== undefined) {
                        balanceMap[supplierId] -= Number(p.amount) || 0;
                    }
                }
            });
            ...
            Object.keys(balanceMap).forEach(key => {
                if (balanceMap[key] < 0 || balanceMap[key] > 1000000000) {
                    balanceMap[key] = 0;
                }
            });
    ...
    const totalDisplayedDebt = useMemo(() => {
        return filteredSuppliers.reduce((sum, s) => {
            ...
            // Protéger contre les valeurs extrêmes
            if (balance > 1000000000 || balance < -1000000000) return sum;
            return sum + balance;
        }, 0);
    }, [filteredSuppliers, balances]);
                    ...
                    <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        ...
                        <tfoot className="border-t-2 border-gray-200 dark:border-gray-700">
                            <tr className="bg-blue-100 dark:bg-blue-900/40">
                                <td colSpan={4} className="px-6 py-4 text-right text-xs font-black uppercase text-primary-600 tracking-widest">
                                    {searchTerm || selectedCity !== 'all' || filterType !== 'all' ? 'Total Dettes (Sélection)' : 'Total Global Dettes (Fournisseurs)'}
                                </td>
                                <td className="px-6 py-4 text-right text-sm font-black text-red-600">
                                    {formatCurrency(totalDisplayedDebt)}
                                </td>
                                <td></td>
                            </tr>
                        </tfoot>
