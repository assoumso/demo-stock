...
const CreditNotesPage: React.FC = () => {
    ...
    const totalPageAmount = useMemo(() => paginatedNotes.reduce((sum, n) => sum + n.amount, 0), [paginatedNotes]);
    const totalGlobalAmount = useMemo(() => filteredNotes.reduce((sum, n) => sum + n.amount, 0), [filteredNotes]);
    ...
    return (
        <div className="p-6">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
                <div>
                    <h1 className="text-2xl font-black text-gray-900 dark:text-white uppercase tracking-tighter">Avoirs & Retours</h1>
                    <p className="text-sm text-gray-500 dark:text-gray-400">Gestion des notes de crédit et retours marchandises</p>
                </div>
                <div className="flex gap-2">
                    <button 
                        onClick={() => setIsListPrintModalOpen(true)}
                        className="flex items-center px-4 py-2 bg-gray-900 hover:bg-black text-white rounded-xl font-bold uppercase text-xs shadow-lg transition-all"
                    >
                        <PrintIcon className="w-4 h-4 mr-2" /> Imprimer Liste
                    </button>
                    <button 
