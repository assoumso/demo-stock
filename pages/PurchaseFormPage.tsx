        ...
        const fetchData = async () => {
            ...
            try {
                ...
                if (isEditing) {
                    ...
                    // We try to find the cost from the product list
                    const purchaseItems: PurchaseItem[] = fromQuote.items.map(qItem => {
                        const product = productsSnap.docs.find(d => d.id === qItem.productId)?.data() as Product;
                        const cost = product?.cost || 0; // Use product cost, not quote price
                        return {
                    ...
                    setFormState(prev => ({
                        ...
                        supplierId: '', // User must select supplier
                        warehouseId: whList.length > 0 ? whList[0].id : '',
                        items: purchaseItems,
                        grandTotal: grandTotal,
                        purchaseStatus: 'En attente', // Default to Pending for new orders
    ...
    const handleFormSubmit = async (e: FormEvent) => {
        ...
        try {
            await runTransaction(db, async (transaction) => {
                ...
                // Update Quote if this came from a quote
                if (fromQuote) {
                     const quoteRef = doc(db, 'quotes', fromQuote.id);
                     transaction.update(quoteRef, {
                         convertedPurchaseId: purchaseRef.id,
                         status: 'Converti'
                     });
                }
            });
                ...
                <div className="overflow-x-auto"><table className="min-w-full">
                    ...
                    <tfoot className="border-t-2 border-gray-200 dark:border-gray-700">
                        <tr className="bg-gray-50 dark:bg-gray-700/50 font-black">
                            ...
                            <td className="px-4 py-3 text-sm text-primary-600">{formatCurrency(formState.items.reduce((sum, item) => sum + item.subtotal, 0))}</td>
                            <td></td>
                        </tr>
                    </tfoot>
                </table></div>
