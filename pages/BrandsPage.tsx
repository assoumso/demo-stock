...
const BrandsPage: React.FC = () => {
    ...
    const openModalForEdit = (item: Brand) => {
        setIsEditing(true);
        setCurrentItem(item);
        setIsModalOpen(true);
    };
    ...
    return (
        <div>
            <div className="flex justify-between items-center mb-6">
                <h1 className="text-2xl font-semibold">Gestion des Marques</h1>
            ...
            <div className="bg-white dark:bg-gray-800 shadow-lg rounded-lg">
                <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                        {items.map(item => (
                            <tr key={item.id} className={selectedIds.includes(item.id) ? 'bg-primary-50 dark:bg-gray-700/50' : ''}>
            ...
            <Modal isOpen={isModalOpen} onClose={closeModal} title={isEditing ? "Modifier la marque" : "Ajouter une marque"}>
                <form onSubmit={handleSave} className="p-6 space-y-4">
                    <input type="text" placeholder="Nom de la marque" value={currentItem.name || ''} onChange={e => setCurrentItem({ ...currentItem, name: e.target.value })} className="w-full border rounded p-2 dark:bg-gray-700"/>
             ...
             <Modal isOpen={isBulkDeleteModalOpen} onClose={() => setIsBulkDeleteModalOpen(false)} title="Confirmer la suppression">
                <div className="p-6"><p>Êtes-vous sûr de vouloir supprimer les {selectedIds.length} marques sélectionnées ?</p></div>
